...
! $Id: main.F90 8 2006-05-06 13:55:59Z cai $
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#include "defines.F90"
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
interface
    function cal_est(direction, i_dir, polar, out_cni, scaling_s, max_rank, &
        maxtime, point, edge)
        real cal_est(:,0:), direction(3), out_cni(:,:,:), scaling_s, point(:,:), &
            maxtime
        integer i_dir, edge(:,:)
    end function cal_est
end interface
! Local variables
real freq, scaling_s, max_r
    ...
    do tri_idx=1,2
        do p_idx=1,3
            s_integral=s_integral+rho(:,p_idx,tri_idx)* &
                psi_func(j_var, time*maxtime/TIME_STEP + &
