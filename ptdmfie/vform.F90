!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! $URL: svn://emn001/projects/ptdemfie/vform.F90 $
! $Id: vform.F90 69 2007-07-18 05:15:54Z cai $
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#include "defines.F90"
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
type(t_triangle) triangle(:)
    ...
    do j_var=0, i_rank-1
        call PGEMM( 'N', 'N', descz(M_), descv(N_), descv(M_), &
            scaling_s, &
            bmnij(:,:,0), 1, 1, descz, out_cni(:,:,j_var), &
        ...
        call PGEMM( 'N', 'N', descz(M_), descv(N_), descv(M_), &
            ...
            amnij(:,:,i_rank-j_var), 1, 1, descz, out_cni(:,:,j_var), &
                1, 1, descv, 1._DKIND, v_rhs, 1, 1, descv )
        call PGEMM( 'N', 'N', descz(M_), descv(N_), descv(M_), &
            scaling_s*.5_DKIND, bmnij(:,:,i_rank-j_var), 1, 1, &
            descz, out_cni(:,:,j_var), 1, 1, descv, 1._DKIND, &
            v_rhs, 1, 1, descv )
        do k_var=0, j_var-1
            call PGEMM( 'N', 'N', descz(M_), descv(N_), descv(M_), &
                scaling_s, &
                bmnij(:,:,i_rank-j_var), 1, 1, descz, out_cni(:,:,k_var), &
                1, 1, descv, 1._DKIND, v_rhs, 1, 1, descv )
        end do
    end do
    end if
